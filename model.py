...
class BiLSTM_CRF(object):
    ...
    def run_one_epoch(self, sess, train, dev, tag2label,epoch, saver):
        ...
        self.logger.info("train lenght={} number_batches={}".format(len(train), num_batches))

        #start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        batches = batch_yield(train, self.batch_size, self.vocab, self.tag2label,shuffle=self.shuffle)
        start_time0 = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        self.logger.info("=========={} epoch begin train, time is {}".format(epoch+1, start_time0))
        for step ,(seq, labels) in enumerate(batches):
            ...
            if step+1 == num_batches:
                self.logger.info("========save session========{}".format(self.model_path))
